...
export default function CrewBannersPage() {
  const [banners, setBanners] = useState<CrewBanner[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingBanner, setEditingBanner] = useState<CrewBanner | null>(null);
  ...
  const loadBanners = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'crew_banners'), orderBy('order'));
      ...
      console.error('Error loading crew banners:', error);
    }
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingBanner) {
        await updateDoc(doc(db, 'crew_banners', editingBanner.id), formData);
      } else {
        await addDoc(collection(db, 'crew_banners'), {
          ...formData,
  ...
  const handleEdit = (banner: CrewBanner) => {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this banner?')) {
      try {
        await deleteDoc(doc(db, 'crew_banners', id));
        loadBanners();
      } catch (error) {
  ...
  const handleToggleActive = async (banner: CrewBanner) => {
    try {
      await updateDoc(doc(db, 'crew_banners', banner.id), { isActive: !banner.isActive });
      loadBanners();
    } catch (error) {
    ...
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Crew Page Banners</h1>
          </div>
          <p className="text-gray-600">Manage the promotional banner shown on the Browse Crew page</p>
        </div>
        <button
          onClick={() => {
            resetForm();
      ...
      <div className="mb-6 rounded-lg bg-blue-50 p-4">
        <p className="text-sm text-blue-700">
          <strong>Recommended Size:</strong> 800 x 200 pixels (4:1 ratio). This banner appears at the top of the Browse Crew page.
        </p>
      </div>
            ...
            {banners.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-gray-500">
                  No crew banners yet. Add one to get started.
                </td>
              </tr>
            ) : (
              banners.map((banner) => (
          ...
          <div className="w-full max-w-lg rounded-lg bg-white p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-bold">
                {editingBanner ? 'Edit Crew Banner' : 'Add Crew Banner'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Find Your Crew"
                />
              </div>
